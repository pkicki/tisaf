...
from models.planner_multiple_endings import plan_loss, _plot, PlanningNetworkMP
...
def main(args):
    ...
    a = 30
    x = np.linspace(-12.5, 0., a)
    b = 30
    y = np.linspace(-20.5, 0., b)
    c = 21
    d = 1
    ...
    for w in [17]:#, 17]:#range(30):
        ...
        cl = tf.stack(curvature_loss, -1)
        il = tf.stack(invalid_loss, -1)
        ol = tf.stack(overshoot_loss, -1)
        l = cl + il + ol
        l = tf.reduce_min(l, -1)
