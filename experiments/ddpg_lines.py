...
from models.actor_lines import plan_loss, _plot, PlanningNetworkMP, calculate_next_point
from models.critic_lines import CriticNetwork
...
def main(args):
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            params = tf.random_normal(tf.shape(params), params, tf.stack([0.1, 0.1], 0)[tf.newaxis])
            p.append(params)
            ...
            if_invalid = tf.cast(invalid_loss != 0, tf.float32)
            # TODO probably need change
            reward = -float(if_invalid) + float(can_finish) - 0.01
            print(reward)
            if can_finish:
                print("FINISHED!!!!")

            xn, yn, thn = calculate_next_point(params, xs, ys, ths)

            episode_finished = k >= N or can_finish or invalid_loss.numpy()[0] > 0.0
            print(invalid_loss)
            print(episode_finished)
            replay_buffer.add((xs, ys, ths, xk, yk, thk), params, reward, (xn, yn, thn, xk, yk, thk),
                              episode_finished)
        ...
        if replay_buffer.count() > 128:
            ...
            xk = tf.stack([x[0][3] for x in data], 0)
            yk = tf.stack([x[0][4] for x in data], 0)
            thk = tf.stack([x[0][5] for x in data], 0)
            ...
            with tf.GradientTape(persistent=True) as tape:
                next_action = target_actor(xn, yn, xk, yk)[:, 0]
                pi_action = actor(x0, y0, xk, yk)[:, 0]
                q = critic(x0, y0, xk, yk, action)
                q_pi = critic(x0, y0, xk, yk, pi_action)
                y = tf.stop_gradient(
                    rewards + gamma * (1 - can_finish[:, tf.newaxis]) * target_critic(xn, yn, xk, yk, next_action))

                print(pi_action[:1, :, tf.newaxis])
                model_loss, invalid_loss, overshoot_loss, curvature_loss, \
