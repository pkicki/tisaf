...
def main(args):
    ...
    # 2. Define model
    model_teacher = PlanningNetworkMP(7, (args.batch_size, 6))
    ...
    # 4. Restore, Log & Save
    eh = ExperimentHandler(args.working_path, args.out_name, args.log_interval, model_teacher, optimizer)
    eh.restore("./working_dir/ten_bez_over_1e-3/checkpoints/best-1022")
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, data in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            ...
            gt_output, gt_last_ddy = model_teacher(data, None, training=True)
            with tf.GradientTape(persistent=True) as tape:
                ...
                output_loss = tf.reduce_sum(output_loss, -1)
                ddy_loss = tf.keras.losses.mean_absolute_error(gt_last_ddy, last_ddy)
                total_loss = output_loss + ddy_loss
            ...
            with tfc.summary.record_summaries_every_n_global_steps(args.log_interval, train_step):
                tfc.summary.scalar('metrics/model_loss', total_loss, step=train_step)
